...
function get_bracket(str) {
	...
	for (i = 0; i < text_list.length; i++) {
		...
		if (/\(/ig.exec(text_list[i]) && is_colon == 0) {
			// 加入with as 的判断可能会有坑
			if (  (/FROM |JOIN|WITH/.exec(last_str) ||  /^\)\s*\,\s*\w+\s*AS/.exec(last_str)  )  && !/ORDER BY/.exec(last_str)) {
				left.push(i);
				bracket.push(i);
				bracket_back.push(1);
...
function extra(str){
	...
	.replace(/\-\-FROMiscomment/ig,"\-\- FROM") //避免关键词注释换行先缩进
}
...
function reshape_comment(str){
	...
	.replace(/\-\- FROM/ig,"\-\-FROMiscomment") //避免关键词注释换行先缩进
	.replace(/\n *\-\-/ig, " \-\-{}")
...
vkbeautify.prototype.sql = function(text,uppercase,comma_location,bracket_char) {
	...
	step4 = except_subquery(step3);
	// step5 = special_wrap(step4).replace(/\-\-\s{0,}\n/ig, "\n-- ");
	step5 = special_wrap(step4).replace(/\s{0,1}--{}/ig, "\n--");
	// console.log(step5);
	step6 = bracket_deep(step5); //union all需要单独函数来考虑
	step7 = extra(step6);
