...
function ddl(str){
	var text_final = '';
	var text_list = [];
	var text_list_orginal = str.replace(/(\d+),(\d+)/ig, "$1#?$2")
								.replace(/\,/ig, "\n,")
							   .replace(/\n\s{1,}/ig, "\n")
...
while (n<text_list.length)
{
	...
	.replace(/\#\?/ig, ',') //æŠŠé€—å·æ›¿æ¢å›žåŽ»
	
	//å¯¹äºŽç©ºè¡Œæ¢è¡Œçš„è¿›è¡Œæ›¿æ¢
}
...
vkbeautify.prototype.sql = function(text,uppercase,comma_location) {
	step1 = reshape_comment(text);
	step2 = replace_char(step1) ;
	step3 = get_bracket(step2);
	step4 = except_subquery(step3);
