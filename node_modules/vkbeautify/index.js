...
function ddl(str){
	var text_final = '';
	var text_list = [];
	var text_list_orginal = str.replace(/\,/ig, "\n,")
							   .replace(/\n\s{1,}/ig, "\n")
...
while (n<text_list.length)
{
	...
	if(is_comment_before == 1 
		&& /^,/ig.exec(text_list[n]) 
		&& /STRING|DOUBLE|BIGINT|INT|VARCHAR/ig.exec(text_list[n]) == null){
	...
	for (i = 0; i < text_list.length; i++) {
		if(/STRING|DOUBLE|BIGINT|INT|VARCHAR/ig.exec(text_list[i]) && text_list[i].indexOf('PARTITIONED') == -1){
...
function reshape_comment(str){
	...
	for (let i = 0; i < text_list_orginal.length; i++) {
		if (text_list_orginal[i] != "" && text_list_orginal[i] != " ") {
			...
			text_list_orginal[i] = a.replace(/(\,)\s+$/ig," ") + " " + text_list_orginal[i].slice(comment_loc,).replace(/\,/ig," ").replace(/\(/ig,"（").replace(/\)/ig,"）");
...
function convert_lowercase(str){
	return str.replace(/AND/g, "and")
		.replace(/THEN/g, "then")
		.replace(/WHEN/g, "when")
		.replace(/INSERT/g, "insert")
		.replace(/INSERT INTO/g, "insert into")
		.replace(/BETWEEN/g, "between")
		.replace(/CASE WHEN/g, "case when")
		.replace(/DATEDIFF/g, "datediff")
		.replace(/CEIL/g, "ceil")
		.replace(/FLOOR/g, "floor")
		.replace(/FROM/g, "from")
		.replace(/JOIN/g, "join")
		.replace(/DISTRIBUTE BY/g, "distribute by")
		.replace(/AS/g, "as")
		.replace(/TABLE/g, "table")
		.replace(/IF EXISTS/g, "if exists")
		.replace(/HAVING/g, "having")
		.replace(/USING/g, "using")
		.replace(/IN/g, "in")
		.replace(/SELECT/g, "select")
		.replace(/WHERE/g, "where")
		.replace(/ON/g, "on")
		.replace(/CROSS JOIN/g, "cross join")
		.replace(/INNER JOIN/g, "inner join")
		.replace(/LEFT JOIN/g, "left join")
		.replace(/RIGHT JOIN/g, "right join")
		.replace(/ORDER BY/g, "order by")
		.replace(/GROUP BY/g, "group by")
		.replace(/GROUPING SETS/g, "grouping sets")
		.replace(/UNION ALL/g, "union all")
		.replace(/LEFT OUTER JOIN/g, "left outer join")
		.replace(/RIGHT OUTER JOIN/g, "right outer join")
		.replace(/FULL OUTER JOIN/g, "full outer join")
		.replace(/FULL JOIN/g, "full join")
		.replace(/DROP/g, "drop")
...
vkbeautify.prototype.sql = function(text,uppercase) {
	step1 = reshape_comment(text);
	step2 = replace_char(step1) ;
	step3 = get_bracket(step2);
	step4 = except_subquery(step3);
